...
모든 세션은 작업 전 `new-pulse-docs` 문서와 로컬 비공개 원천 자료의 요구·데이터·제출 제약을 먼저 이해해야 한다. 단, 원천 자료의 내용은 Public 저장소 문서나 코드에 복사하지 않는다.
...
13. 로컬 비공개 원천 자료의 요구사항과 사용자 데이터 형태
14. 제출 및 공개 저장소 운영 제약 요약

제출 및 공개 제약 요약:
- 상세 요구사항과 샘플 데이터는 로컬 비공개 원천 자료를 기준으로 확인한다.
- 제출물은 평가자가 접근 가능한 공개 Git 저장소 형태로 준비한다.
- 원천 자료의 본문, 원본 데이터, 안내 문구는 외부 공개 문서와 코드에 복사하지 않는다.
- 별도 질의 없이 검토 가능한 수준으로 README에 실행 방법, 검증 방법, 설계 판단을 남긴다.

공통 원칙:
- 과제 원문, 원본 Excel, 제출 안내 원문을 README/코드/공개 문서에 복사하지 마라.
...
- 커밋 메시지는 한글로 작성하라. 타입은 Conventional Commits 형식(`feat:`, `fix:`, `docs:` 등)을 유지해도 되지만 설명은 한글이어야 한다.
...
- 과제 원문, 제출 안내 원문, 원본 Excel을 README에 복사하지 않는다.
